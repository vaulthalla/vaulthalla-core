...
void InternalSecretQueries::upsertSecret(const std::shared_ptr<InternalSecret>& secret) {
...
std::shared_ptr<InternalSecret> InternalSecretQueries::getSecret(const std::string& key) {
    return Transactions::exec("InternalSecretQueries::getSecret", [&](pqxx::work& txn) -> std::shared_ptr<InternalSecret> {
        const auto res = txn.exec(pqxx::prepped{"get_internal_secret"}, key);
        if (res.empty()) return nullptr;
        return std::make_shared<InternalSecret>(res.one_row());
    });
}
