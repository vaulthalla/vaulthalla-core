...
std::unordered_map<std::string, std::string> CloudStorageEngine::getMetaMapFromFile(const std::shared_ptr<File>& f) const {
...
void CloudStorageEngine::upload(const std::shared_ptr<File>& f) const {
...
void CloudStorageEngine::upload(const std::shared_ptr<File>& f, const std::vector<uint8_t>& buffer, const bool isCiphertext) const {
...
std::vector<uint8_t> CloudStorageEngine::downloadToBuffer(const fs::path& rel_path) const {
    std::vector<uint8_t> buffer;
    s3Provider_->downloadToBuffer(stripLeadingSlash(rel_path), buffer);
    return buffer;
}

std::shared_ptr<File> CloudStorageEngine::downloadFile(const fs::path& rel_path) {
...
void CloudStorageEngine::indexAndDeleteFile(const fs::path& rel_path) {
...
CloudStorageEngine::getGroupedFilesFromS3(const fs::path& prefix) const {
    return groupEntriesByPath(filesFromS3XML(s3Provider_->listObjects(prefix)));
}

std::string CloudStorageEngine::getRemoteContentHash(const fs::path& rel_path) const {
...
bool CloudStorageEngine::remoteFileIsEncrypted(const fs::path& rel_path) const {
    const auto s3Key = stripLeadingSlash(rel_path);
...
std::optional<std::pair<std::string, unsigned int>> CloudStorageEngine::getRemoteIVBase64AndVersion(const fs::path& rel_path) const {
...
void CloudStorageEngine::purge(const fs::path& rel_path) const {
    removeLocally(rel_path);
    removeRemotely(rel_path, true);
}

void CloudStorageEngine::purge(const std::shared_ptr<TrashedFile>& f) const {
    removeLocally(f);
    removeRemotely(f, true);
}

void CloudStorageEngine::removeLocally(const fs::path& rel_path) const {
...
void CloudStorageEngine::removeLocally(const std::shared_ptr<TrashedFile>& f) const {
...
void CloudStorageEngine::removeRemotely(const fs::path& rel_path, const bool rmThumbnails) const {
    s3Provider_->deleteObject(stripLeadingSlash(rel_path));
    if (rmThumbnails) purgeThumbnails(rel_path);
}

void CloudStorageEngine::removeRemotely(const std::shared_ptr<types::TrashedFile>& f, bool rmThumbnails) const {
    const auto vaultPath = paths->absRelToAbsRel(f->path, PathType::FUSE_ROOT, PathType::VAULT_ROOT);
