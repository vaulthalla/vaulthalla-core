...
void to_json(nlohmann::json& j, const File& f) {
    ...
    if (f.parent_id) j["parent_id"] = *f.parent_id;
    if (f.trashed_at) j["trashed_at"] = *f.trashed_at;
}
...
void from_json(const nlohmann::json& j, File& f) {
    ...
    f.created_at = util::parseTimestampFromString(j.at("created_at").get<std::string>());
    f.updated_at = util::parseTimestampFromString(j.at("updated_at").get<std::string>());
    f.current_version_size_bytes = j.at("current_version_size_bytes").get<unsigned long long>();
    f.is_trashed = j.at("is_trashed").get<bool>();
    f.trashed_at = util::parseTimestampFromString(j.at("trashed_at").get<std::string>());
