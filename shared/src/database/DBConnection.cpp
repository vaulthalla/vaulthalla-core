#include "../../../shared/include/database/DBConnection.hpp"
...
void DBConnection::initPreparedFsEntries() const {
    ...
    conn_->prepare("upsert_fs_entry",
                   "INSERT INTO fs_entry (vault_id, parent_id, name, path, abs_path, backing_path, inode, mode, owner_uid, group_gid, is_hidden, is_system) "
...
void DBConnection::initPreparedFiles() const {
    ...
    conn_->prepare("upsert_file_full",
                   ...
                   "  INSERT INTO fs_entry (vault_id, parent_id, name, created_by, last_modified_by, path, abs_path, backing_path, inode, mode, owner_uid, group_gid, is_hidden, is_system) "
                   ...
                   "SELECT id, $15, $16, $17, $18 FROM upsert_entry "
                   "ON CONFLICT (fs_entry_id) DO UPDATE SET "
                   "  size_bytes = EXCLUDED.size_bytes, "
                   "  mime_type = EXCLUDED.mime_type, "
                   "  content_hash = EXCLUDED.content_hash, "
...
void DBConnection::initPreparedDirectories() const {
    ...
    conn_->prepare("upsert_directory",
                   ...
                   "SELECT id, $15, $16, $17, NOW() FROM inserted "
                   "ON CONFLICT (fs_entry_id) DO UPDATE SET "
                   "  size_bytes = EXCLUDED.size_bytes, "
