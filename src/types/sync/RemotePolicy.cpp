...
void RemotePolicy::preflightSpaceForPlan(const std::weak_ptr<concurrency::SyncTask>& ctx, const std::vector<Action>& plan) const {
    auto task = ctx.lock();
    ...
    if constexpr (requires { SyncTask::computeReqFreeSpaceForDownload(downloads); }) {
        required = SyncTask::computeReqFreeSpaceForDownload(downloads);
    } else {
