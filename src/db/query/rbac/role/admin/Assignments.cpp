...
namespace vh::db::query::rbac::role::admin {
    ...
    uint32_t Assignments::countAssignmentsForRole(uint32_t role_id) {
        return Transactions::exec("role::admin::Assignments::countAssignmentsForRole(role_id)", [&](pqxx::work& txn) -> uint32_t {
            const auto result = txn.exec(
                pqxx::prepped{"count_admin_role_assignments_by_role_id"},
                pqxx::params{role_id}
            );
