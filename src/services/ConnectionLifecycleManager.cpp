...
namespace vh::services {
    ...
    void ConnectionLifecycleManager::sweepActiveSessions() {
        ...
        for (const auto& [tokenStr, client] : sessions) {
            ...
            if (token->revoked) {
                std::cout << "[LifecycleManager] Token revoked. Closing session for user " << client->getUser()->id << std::endl;
            ...
            if (secondsLeft <= 0) {
                std::cout << "[LifecycleManager] Token expired. Closing session for user " << client->getUser()->id << std::endl;
            ...
            if (secondsLeft <= 10) {
                client->sendControlMessage("token_refresh_urgent", {
                        {"deadline_ms", 10000}
                });
            } else if (secondsLeft <= 300) {
                client->sendControlMessage("token_refresh_requested", {
                        {"deadline_ms", 300000}
                });
            }
        }
