...
namespace vh::cloud::s3 {
    ...
    public:
        ...
        bool uploadLargeObject(const std::string& bucket, const std::string& key, const std::string& filePath, size_t partSize = 5 * 1024 * 1024);

        std::string initiateMultipartUpload(const std::string& bucket, const std::string& key);
        bool uploadPart(const std::string& bucket, const std::string& key, const std::string& uploadId, int partNumber, const std::string& partData, std::string& etagOut);
        bool completeMultipartUpload(const std::string& bucket, const std::string& key, const std::string& uploadId, const std::vector<std::string>& etags);
        bool abortMultipartUpload(const std::string& bucket, const std::string& key, const std::string& uploadId);

    private:
        std::string buildAuthorizationHeader(const std::string& method, const std::string& canonicalUri, const std::map<std::string, std::string>& headers, const std::string& payloadHash);
