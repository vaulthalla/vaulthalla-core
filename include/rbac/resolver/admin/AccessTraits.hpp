...
namespace vh::rbac::resolver {
    ...
    template<>
    struct AccessTraits<permission::admin::keys::APIKeyBase> {
    ...
    template <>
    struct AccessTraits<permission::admin::identities::IdentityPermissions> {
        static const auto& direct(const role::Admin& role) { return role.identities; }
        static const auto& groups(const decltype(std::declval<identities::User>().identities().groups)& perms) { return perms; }
        static const auto& admins(const decltype(std::declval<identities::User>().identities().admins)& perms) { return perms; }
        static const auto& users(const decltype(std::declval<identities::User>().identities().users)& perms) { return perms; }
    };
