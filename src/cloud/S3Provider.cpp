...
// --- ctor/dtor ------------------------------------------------------------
S3Provider::S3Provider(const std::shared_ptr<types::api::S3APIKey>& apiKey)
        : apiKey_(apiKey) {
...
std::string S3Provider::hmacSha256Hex(const std::string& key, const std::string& data) {
    ...
    std::ostringstream oss;
    for (int i = 0; i < SHA256_DIGEST_LENGTH; i++) oss << std::hex << std::setw(2) << std::setfill('0') << (int)digest[i];
    return oss.str();
}

std::string S3Provider::buildAuthorizationHeader(
    ...
    if (qpos != std::string::npos) {
        ...
        if (canonicalQuery.find('=') == std::string::npos)
            canonicalQuery += '=';            // ensure "uploads=" not "uploads"
    }
    ...
    HMAC(EVP_sha256(), kSigning.data(), kSigning.size(),
         reinterpret_cast<const unsigned char*>(stringToSign.data()), stringToSign.size(), sigRaw, nullptr);
...
std::string S3Provider::initiateMultipartUpload(const std::string& bucket, const std::string& key) {
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, +[](char* p,size_t s,size_t n,std::string* d){ d->append(p,s*n); return s*n; });
    ...
    if (res != CURLE_OK || httpCode != 200) {
        std::cerr << "initiateMultipartUpload failed: CURL=" << res << " HTTP=" << httpCode << "Resp: " << response << std::endl;
        return "";
    }

    std::smatch m; std::regex re("<UploadId>([^<]+)</UploadId>");
...
bool S3Provider::uploadPart(const std::string& bucket,const std::string& key,const std::string& uploadId,int partNumber,const std::string& partData,std::string& etagOut){
    ...
    std::map<std::string,std::string> hdrMap{{"host",apiKey_->endpoint.substr(apiKey_->endpoint.find("//")+2)},{"x-amz-content-sha256",payloadHash},{"x-amz-date",vh::util::getCurrentTimestamp()}};
    ...
    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, +[](char* b,size_t s,size_t n,std::string* out){ out->append(b,s*n); return s*n; });
...
bool S3Provider::completeMultipartUpload(const std::string& bucket,const std::string& key,const std::string& uploadId,const std::vector<std::string>& etags){
    CURL* curl=curl_easy_init();
...
// -------------------------------------------------------------------------
bool S3Provider::uploadLargeObject(const std::string& bucket,const std::string& key,const std::string& filePath,size_t partSize){
    std::ifstream file(filePath,std::ios::binary);
    if(!file) return false;
