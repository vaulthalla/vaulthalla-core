...
class ShareLink {
  public:
    ShareLink() = default;
    ShareLink(std::string ownerUsername, std::string mountName, std::string path, std::string permissionType,
              std::optional<std::chrono::system_clock::time_point> expiresAt = std::nullopt);

    ShareLink(std::string linkId, std::string ownerUsername, std::string mountName, std::string path,
