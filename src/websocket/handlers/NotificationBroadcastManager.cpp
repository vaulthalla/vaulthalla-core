...
namespace vh::websocket {
    ...
    void NotificationBroadcastManager::broadcastNotification(const std::string& channel,
                                                             const nlohmann::json& payload) {
        std::lock_guard<std::mutex> lock(sessionsMutex_);

        std::cout << "[NotificationBroadcastManager] Broadcasting to channel '" << channel
                  << "' to " << sessions_.size() << " session(s).\n";
