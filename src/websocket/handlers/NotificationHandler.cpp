...
void NotificationHandler::handleSubscribe(const json& msg, WebSocketSession& session) {
    try {
        ...
        json response = {{"command", "notification.subscribe.response"}, {"status", "ok"}, {"channels", channels}};
...
void NotificationHandler::handleUnsubscribe(const json& msg, WebSocketSession& session) {
    try {
        ...
        json response = {{"command", "notification.unsubscribe.response"}, {"status", "ok"}, {"channels", channels}};
...
void NotificationHandler::pushNotification(WebSocketSession& session, const std::string& channel, const json& payload) {
    if (!session.isSubscribedTo(channel)) return; // Session not subscribed → no push

    json message = {{"command", "notification.push"}, {"channel", channel}, {"payload", payload}};
