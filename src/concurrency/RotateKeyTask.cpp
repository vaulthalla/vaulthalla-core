...
RotateKeyTask::RotateKeyTask(std::shared_ptr<StorageEngine> eng,
...
bool RotateKeyTask::shouldSkipLocalWriteInCacheMode(const RemotePolicySP& policy,
                                                    const std::size_t ciphertextSize) const {
    if (!policy) return false;
    if (policy->strategy != sync::RemotePolicy::Strategy::Cache) return false;
    return (ciphertextSize * 2) < engine->freeSpace();
}

std::vector<uint8_t> RotateKeyTask::produceCiphertext(const FileSP& file,
...
void RotateKeyTask::hydrateIvAndVersionForRemoteEncrypted(const FileSP& file) const {
...
void RotateKeyTask::maybeWriteLocal(const RemotePolicySP& policy,
                                   const FileSP& file,
...
void RotateKeyTask::rotateLocalFile(const FileSP& file) const {
...
void RotateKeyTask::rotateCloudFile(const RemotePolicySP& remotePolicy,
...
void RotateKeyTask::operator()() {
    try {
        const auto remotePolicy = std::dynamic_pointer_cast<sync::RemotePolicy>(engine->sync);
